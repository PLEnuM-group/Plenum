...
def aeff_rotation(
    coord_lat, coord_lon, eq_coords, grid2d, _ra_width, log_aeff=False, return_3D=False
):
    """
    ...
    if log_aeff:
        _aeff = np.array(
            [
                np.exp(_grid((np.sin(local_coords.alt.rad), local_coords.az.rad)))
                * _ra_width
                for _grid in grid2d
            ]
        )
        ...
        _aeff = np.array(
            [
                _grid((np.sin(local_coords.alt.rad), local_coords.az.rad)) * _ra_width
                for _grid in grid2d
            ]
        )
    if return_3D:
...
if __name__ == "__main__":
    ...
    smth_aeff_spl = RectBivariateSpline(x, y, GD1.T, s=0)
